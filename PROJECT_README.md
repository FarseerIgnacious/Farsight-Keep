...
- ✅ XML import (Game Master 5 format)
...
#### Code Quality
- ✅ All debug logs removed (except migrations)
- ✅ No syntax errors
- ✅ Size abbreviations expanded everywhere
- ✅ Shared utilities (expandSize, parseDamageString, circleToggle)

### Known Limitations ⚠️

1. **No Undo/Redo**: Edits are immediate, no history stack
2. **No Cloud Sync**: All data is local (by design)
3. **No Multi-Window Support**: One window at a time
4. **No Auto-Backup**: User must manually export to XML
5. **No Search in Notes**: Notes are browsable but not searchable
6. **No Dice Roll History**: Rolls are instant, not logged

### Recent Improvements (Last Session)

- ✅ Fixed spell picker Back button styling
- ✅ Universal size abbreviation expansion (T→Tiny, etc.)
- ✅ Removed 73 debug console.log statements
- ✅ Deleted 95 diagnostic .md files
...
### Immediate: Distribution Packaging

**Goal**: Package the app for macOS/Windows/Linux distribution
...
1. Configure electron-builder in package.json:
   ```json
   "build": {
     "appId": "com.dmscompanion.app",
     "productName": "Farsight Keep",
     "mac": {
...
- [ ] Encounter templates (save/load common encounters)
- [ ] Character portraits drag-and-drop
- [ ] Bulk import from CSV
- [ ] Condition duration tracking (countdown)
- [ ] Auto-save draft on close (prompt to resume)
- [ ] Keyboard shortcuts (Ctrl+S to save, etc.)
- [ ] Export to PDF (stat blocks)
...
- [ ] Lazy load encounter cards (render only visible)

**Code Improvements:**
- [ ] TypeScript migration (type safety)
- [ ] Unit tests for parsers (damage, attack, speed)
- [ ] Integration tests for builders (Playwright/Spectron)
...
1. Define in renderer.js
2. Expose on window: `window.myUtility = myUtility`
3. Use in builders: `window.myUtility(args)`

**Add a new builder section:**
1. Create render function: `function mbRenderNewSection() { return '...' }`
2. Add to form: `${mbCardWrap('TITLE', mbRenderNewSection())}`
3. Add to draft model: `mb.draft.newField = defaultValue`
4. Add to save/load: Update `mbBuildCompendiumEntry()` and `mbFromCompendium()`

**Debug data issues:**
1. Open DevTools: View → Toggle Developer Tools
2. Check LocalStorage: Application → Local Storage
3. Inspect compendiumData: Console → `compendiumData`
4. Check migrations: Look for console.log with `[Migration]` prefix

### Testing Checklist

Before each release:
- [ ] Create monster from scratch
- [ ] Edit existing monster
- [ ] Import XML (test with sample file)
- [ ] Export XML (verify re-import works)
- [ ] Create campaign → add PCs → run encounter
- [ ] Test combat: roll attacks, track HP, manage conditions
- [ ] Switch campaigns (ensure data isolates correctly)
- [ ] Delete content (monster, NPC, PC, spell, campaign)
...
### Data corruption / LocalStorage issues
```bash
...
- **Quotes**: Single quotes for strings (except HTML attributes)
- **Semicolons**: Use them
- **Naming**:
  - Functions: camelCase (`renderMonsterBuilder`)
  - Constants: UPPER_SNAKE_CASE (`MB_SIZES`)
  - File-scoped state: camelCase object (`mb`, `enc`, `compendiumData`)
  - Window exports: Prefix with file initial (`mbSave`, `pcbRenderSpells`)

### HTML (in template strings)

- **Indentation**: Match surrounding JavaScript indentation
- **Attributes**: Double quotes
- **Event handlers**: Inline onClick/onChange (no event delegation)
- **IDs**: kebab-case (`mb-combat-section`)
- **Styles**: Inline (no CSS files)

### Comments

- **Functions**: JSDoc-style for exported functions
- **Sections**: `// ── Section Name ──────────────────────────`
- **Inline**: Only for non-obvious "why" (not "what")
